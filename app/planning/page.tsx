...
function PlanningInner() {
  ...
  const load = useCallback(async () => {
    const [{ data: cp }, wxRes] = await Promise.all([
      ...
      fetch('https://api.open-meteo.com/v1/forecast?latitude=18.992&longitude=98.972&daily=precipitation_probability_max&timezone=Asia%2FBangkok&forecast_days=14').then(r=>r.json()).catch(()=>null),
    ]);
    setPlans(cp||[]);
    if (wxRes?.daily) {
